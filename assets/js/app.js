...
async function handleSendMessage() {
    if (!currentChatId || !messageInput || !currentUser) return;  // FIX: guard currentUser
...
async function handleUserSearch(query) {
    if (!currentUser) return;   // FIX: guard
    try {
...
async function handleStartChat(user) {
    if (!currentUser) { showToast('Not logged in.', 'error'); return; }  // FIX: guard
    try {
