...
export const searchUsers = async (searchTerm, currentUserId, maxResults = 10) => {
    ...
    if (!currentUserId) return [];   // FIX: guard against null uid

    const term = searchTerm.replace(/^@/, '').toLowerCase().trim();

    // FIX: search username, email AND display_name so users are easier to find
    const { data, error } = await supabase
        .from('users')
        .select('*')
        .neq('id', currentUserId)
        .or(`username.ilike.%${term}%,email.ilike.%${term}%,display_name.ilike.%${term}%`)
        .limit(maxResults);

    if (error) { console.error('searchUsers error:', error.message, error.code); return []; }
